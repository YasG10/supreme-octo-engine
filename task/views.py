...
def signup(request):
    if request.method == "GET":
        ...
        return render(
            request,
            "signup.html",
            {"form": UserCreationForm, "error": "Password no coinciden"},
        )
...
@login_required
def task_datails(request, task_id):
    if request.method == "GET":
        task = get_object_or_404(Task, pk=task_id, user=request.user)
        form = TaskForm(instance=task)
        return render(request, "task_datails.html", {
            'task': task, 
            'form': form,})
    else:
        try:
    ...
    if request.method == "GET":
        ...
        if user is None:
            return render(
                request,
                "signin.html",
                {"form": AuthenticationForm, "error": "Usuario o Password incorrecto"},
            )
        else:
            login(request, user)
            return redirect("task")
